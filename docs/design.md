...
osslab-agent 的解法：**把 AI agent 變成可跨 channel 工作的同事**。Lark 負責一次性行動、通知、審核與人工介入；cc web 負責較長的研究、檔案 / 圖片輸入、stream 對話、知識整理，並在 web 內用 project context 把 research / dev / it 等不同上下文切開；Chrome 容器負責登入真實系統、操作 ERP / Email / 電商 / 文件工具。遇到驗證碼 / 2FA / 要刷信用卡求救，人類點兩下接手 5 秒搞定。

**這跟 openclaw / harmes 方向不同。**osslab-agent 不是再打造一套自研 agent framework，也不是重做模型調度核心；它把現成的 Claude Code CLI、Codex CLI 或其他 code agent CLI 當成可替換的 agent process。osslab-agent 自己負責 channel routing、session 管理、cc web 長對話、Lark 審核、Chrome 容器、人工接手與稽核紀錄。這樣可以吃到訂閱制 CLI 的成本優勢，又保留自架工作流與資料邊界。
...
簡單說：**Lark 是行動與審核入口；cc web 是研究、沉澱與 project context 的主入口。**
...
所有事情都可以從 Lark 觸發，但不是所有事情都應該留在 Lark。短任務在 Lark 完成最自然；研究與知識型工作則應進 cc web，最後只把摘要、決策點、待辦或需要人工介入的部分回寫 Lark。
...
# 4. 為什麼這套組合能做到上面這些 — Lark + cc web + cc-connect + code agent + CDP

看完應用範圍你大概會問：「這些事 AI 早就能做了，有什麼特別？」關鍵不是單一模型，而是**channel、runtime、工具、瀏覽器、知識沉澱**被串成一條可審核的工作流。

Larksuite 把 IM、Email、雲端文件、行事曆、任務、視訊全塞在同一套 OpenAPI 後面；lark-cli 把這套 OpenAPI 包成 shell 指令，AI agent 一句話就能用，不用你自己拼 IMAP / Calendar / Doc 等十幾個 SDK。cc web 則提供另一個正式入口：它是 cc-connecter 的改版 web channel，支援 stream、圖片、檔案和長對話，讓 research / knowledge / project 類工作不必硬塞在 Lark 訊息流裡。

cc-connect 在這裡不只是「Lark 訊息轉 Claude Code」的 bridge，而是 channel router：Lark 進來的任務、cc web 進來的長對話、cc web 內的 project context，都會被送到合適的 code agent session。agent 可以走 Claude Code CLI、Codex CLI 這類訂閱制 CLI，降低 API 成本；未來也可以把部分任務切到本地 LLM，讓敏感資料留在本機或內網。
...
這正是其他自動化方案最常缺的最後一塊拼圖：headless puppeteer / 純 RPA 腳本卡關時只能 fail，這套方案**卡關時就求救人類，人類處理完 AI 無縫接續**。多入口疊起來，AI agent 才能像真同事一樣：在 Lark 裡接短任務和審核，在 cc web 裡做研究、整理與 project 分流，在 Chrome 裡操作真實系統。

本版本以 **Claude Code（Anthropic 官方 CLI）**作為第一個 code agent runtime，並把 **Codex CLI** 視為同一類可接 runtime：搭配個人或團隊訂閱制即可使用——不需要每個任務都走純 API key、不需要把成本全部壓到 Bedrock / Vertex / OpenAI API 帳單。其他訂閱制 CLI、本地 LLM、私有檢索會作為同一架構下的擴展方向。

這個設計也不依賴單次 prompt 的 `-p` 類模式。當某些 CLI 的一次性 prompt 模式不穩定、被限制或不再適合自動化時，cc-connect / cc web 仍可用長 session、stdio、process bridge 或互動式 session 管理 agent，保留上下文、stream 回傳與工具調用。

架構天生支援**多人 + AI 同事 + 多 channel 協作**：多個真人 @ 同一個 Lark bot、多個 bot 互相 @、單人單 bot、cc web 長對話、project 分流都能共存。所有需要人類決策的操作可以回到 Lark 留下審核紀錄；cc-connect 後台同時保留每個 session 的詳細日誌、工具呼叫紀錄、stdin/stdout；cc web 內保留研究 artifact 與知識版本，形成可追溯紀錄。
