# osslab-agent — 多入口 AI 同事協同作業架構

> [!NOTE]
> 🤝
> **osslab-agent 是什麼：一套自架的多入口 AI 同事架構。**
> 它把 Larksuite 群組、cc web 對話、project workspace、Chrome 容器、知識庫與訂閱制 code agent 串成同一套工作流。短任務可以在 Lark 裡完成；研究、比價、資料整理與知識沉澱則進 cc web / project workspace；需要登入真實系統時再由 Chrome 容器接手。
> 核心策略是利用各家類訂閱制 code 作為 agent（可合規使用訂閱制，減少 AI API 成本），必要時也能接本地 LLM / 私有知識庫，讓敏感資料留在自己控制的環境內。
> AI 不是按鈕、不是單一聊天視窗，而是**能存在於不同 channel 的同事**：能在 Lark 裡被 @，能在 web 裡做研究，能進 project workspace 沉澱資料，也能操作真實 Chrome 查 ERP、寫文件、reply email、解 captcha 時求救。
...
但另一類工作也越來越常見：**查閱與研究外部資訊、比價、整理來源、把關鍵結論沉澱成知識庫、再依 project 分流到 research / dev / it 等不同上下文**。這些事情可以在 Lark 裡對談完成，但如果全部只留在 IM 訊息流，長期會很難沉澱：來源散落、檔案和圖片混在對話裡、結論版本不清、後續要接本地 LLM / 私有 KB 時 retrieval 會很髒。

傳統做法：每人開一堆分頁手動切換，或寫一堆 RPA 腳本（脆弱、難維護、跑不起來時沒人懂）。

osslab-agent 的解法：**把 AI agent 變成可跨 channel 工作的同事**。Lark 負責一次性行動、通知、審核與人工介入；cc web 負責較長的研究、檔案 / 圖片輸入、stream 對話與知識整理；project workspace 負責把 research / dev / it 等不同上下文切開；Chrome 容器負責登入真實系統、操作 ERP / Email / 電商 / 文件工具。遇到驗證碼 / 2FA / 要刷信用卡求救，人類點兩下接手 5 秒搞定。

**單人也用、團隊也用——關鍵是「一人 N agent / N bot」都成立。**單人創業可以一個人配 N 個 agent——採購、客服、業務、市場、research、dev、it——等於一個人就擁有一支 AI 助理小隊；團隊版同樣**每個業務角色 / 每個工作流 / 每個 project 配一個獨立 agent**。Lark 裡的 bot 負責對人交辦與審核；cc web 裡的 agent 負責長對話、研究與資料整理；需要登入狀態隔離時再對應到獨立 Chrome 容器。**agent 數量跟人數沒綁**，採購 bot 一隻 vs 多隻、市場 research 要不要分海內外、dev / it 要不要分 project——你自己看工作流配。各 agent 各跑各的，但關鍵交辦、操作、結果都能回到 Lark 或 project log 裡，形成可審計、可重播的紀錄。
...
cc web 在 osslab-agent 裡是正式入口，不是附屬功能：
...
- **知識沉澱**：關鍵結果寫回 project workspace / KB，之後可接本地 LLM 做私有檢索。
- **回寫 Lark**：需要人類決策、審核、付款、登入或對外送出時，再把摘要與行動項回到 Lark。

簡單說：**Lark 是行動與審核入口；cc web 是研究與沉澱入口；KB / project workspace 是長期記憶。**
...
| **Knowledge 層** | cc web + KB | 關鍵資料沉澱、FAQ、公司流程、專案知識 | 可檢索知識、可接本地 LLM |
| **Project 層** | project workspace | research / dev / it 等上下文分流 | project log / task / code or infra context |

所有事情都可以從 Lark 觸發，但不是所有事情都應該留在 Lark。短任務在 Lark 完成最自然；研究與知識型工作則應進 cc web / project workspace，最後只把摘要、決策點、待辦或需要人工介入的部分回寫 Lark。
...
- 研究結果先形成 artifact，再決定是否沉澱到 KB 或回寫 Lark
- 需要付款、登入、送信、建單時才切回 Lark 審核或 Chrome 容器執行

## 3.2 Knowledge / KB（關鍵資料沉澱）
...
- 不同 project 有不同工具、文件、KB、權限與預設工作模式
...
# 4. 為什麼這套組合能做到上面這些 — Lark + cc web + cc-connect + code agent + CDP + KB

看完應用範圍你大概會問：「這些事 AI 早就能做了，有什麼特別？」關鍵不是單一模型，而是**channel、runtime、工具、瀏覽器、知識沉澱**被串成一條可審核的工作流。

Larksuite 把 IM、Email、雲端文件、行事曆、任務、視訊全塞在同一套 OpenAPI 後面；lark-cli 把這套 OpenAPI 包成 shell 指令，AI agent 一句話就能用，不用你自己拼 IMAP / Calendar / Doc 等十幾個 SDK。cc web 則提供另一個正式入口：它支援 stream、圖片、檔案和長對話，讓 research / knowledge / project 類工作不必硬塞在 Lark 訊息流裡。

cc-connect 在這裡不只是「Lark 訊息轉 Claude Code」的 bridge，而是 channel router：Lark 進來的任務、cc web 進來的長對話、project workspace 的上下文，都會被送到合適的 code agent session。agent 可以走 Claude Code 這類訂閱制 CLI，降低 API 成本；未來也可以把部分任務切到本地 LLM，讓私有 KB 和敏感資料留在本機或內網。

需要操作真實系統時，Claude Code 在外面接 MCP（Playwright MCP server）負責決策跟工具調用；MCP 透過 **CDP（Chrome DevTools Protocol）**直接驅動 Chrome 容器，沒 OpenAPI 的網站也能進得去；最後 **KasmVNC web VNC 把同一個 Chrome 容器再以 web 推送出來**——AI 跟真人**共用同一個瀏覽器、同一個登入 session、同一份 cookie**。AI 跑到一半遇到要刷信用卡、過 hCaptcha、解 2FA、第一次登入新網站，人類從 Mac / iPad / 手機隨便一台打開 web VNC 接手 5 秒搞定，搞完 AI 接著做不會 session 斷。

這正是其他自動化方案最常缺的最後一塊拼圖：headless puppeteer / 純 RPA 腳本卡關時只能 fail，這套方案**卡關時就求救人類，人類處理完 AI 無縫接續**。多入口疊起來，AI agent 才能像真同事一樣：在 Lark 裡接短任務和審核，在 cc web 裡做研究和整理，在 project workspace 裡保存上下文，在 Chrome 裡操作真實系統。

本版本以 **Claude Code（Anthropic 官方 CLI）**作為第一個 code agent runtime，搭配個人或團隊 Claude 訂閱制即可使用——不需要你自己管 API key、不需要 Bedrock / Vertex 帳單。其他訂閱制 CLI、本地 LLM、私有 KB 會作為同一架構下的擴展方向。

架構天生支援**多人 + AI 同事 + 多 channel 協作**：多個真人 @ 同一個 Lark bot、多個 bot 互相 @、單人單 bot、cc web 長對話、project 分流都能共存。所有需要人類決策的操作可以回到 Lark 留下審核紀錄；cc-connect 後台同時保留每個 session 的詳細日誌、工具呼叫紀錄、stdin/stdout；project workspace 則保留研究 artifact 與知識版本，形成三層溯源。

# 5. 技術架構

從一條 Lark 訊息、cc web 對話或 project task 到網頁實際被點擊，經過幾層 daemon 跟 protocol：
...
cc web 對話 / 圖片 / 檔案 / stream
  ↓ HTTP/WebSocket
project workspace / KB context
  ↓ local file / database / vector index
cc-connect channel router (systemd --user)
  ↓ session routing + JSONL stdio (ACP-like 協議)
code agent runtime (Claude Code first; subscription CLI 優先)
...
| **cc web** | 正式 web 對話入口，支援 stream、圖片、檔案、長研究任務與 artifact 整理 | 本架構核心入口 |
| **project workspace / KB** | 保存 research / dev / it 等 project context、知識版本、來源與可檢索資料 | 本機 / 自架 |
...
| **Claude Code** | 第一個 code agent runtime（用個人 / 團隊訂閱合法使用）— 真正的 AI agent 大腦 | 外部（建議訂閱制） |
| **本地 LLM / 私有檢索** | 用於敏感 KB、內部文件摘要、低成本或離線查詢 | 可選擴展 |
...
> [!NOTE]
> 💡
> AI agent 能做事是雙面刃——能查 ERP、能寄 email，意思是**權限沒切好就能闖禍**。本架構的資安主要從三個面向控管：Larksuite 後台 + chrome 容器密碼策略 + project / KB 資料邊界。
...
## 7.3 Project / KB：知識邊界與本地 LLM

cc web 會承接研究、圖片、檔案和長對話，因此不能把所有資料丟進同一個無邊界記憶池。建議把知識沉澱切成 project：
...
每個 project 有自己的資料來源、工具允許清單、KB 索引與保留策略。敏感資料可以只進本地檔案 / 本地向量索引 / 本地 LLM，不送外部 API；需要外部模型處理時，先摘要、遮蔽或只送非敏感片段。
...
8. 建立 project workspace / KB 目錄，讓 research / dev / it 等上下文可以分流
...
- 非 Claude Code 的 code agent runtime（Gemini CLI / Codex / 開源模型，v2+ 計畫）
- 本地 LLM 的完整部署包（架構預留；v1 先把 KB / project 邊界設好）
